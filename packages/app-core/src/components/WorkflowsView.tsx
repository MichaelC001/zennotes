...
function opLabel(op: WorkflowOp): string {
  switch (op.kind) {
...
function opPath(op: WorkflowOp): string | null {
  return 'path' in op ? op.path : null
}

const IRREVERSIBLE_KINDS: ReadonlySet<WorkflowOp['kind']> = new Set(['notify', 'clipboard'])

/** Enough paths to recognize what a line covers, not a whole vault in a tooltip. */
const TOOLTIP_PATHS = 20

function pathsTooltip(summary: OpSummary): string | undefined {
...
function summarizeOps(ops: readonly WorkflowOp[]): OpSummary[] {
    ...
    if (withTemplates.missing.length > 0) {
      ...
      setRunError(
        withTemplates.missing.length === 1
          ? `Cannot run: ${names} is not a template in this vault.`
          : `Cannot run: ${names} are not templates in this vault.`
      )
      return
    }
